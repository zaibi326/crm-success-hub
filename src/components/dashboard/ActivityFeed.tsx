    ...
    <Card className="bg-white/70 backdrop-blur-sm border-white/50 shadow-lg transition-all duration-300 hover:shadow-xl">
      <CardHeader className="pb-4">
        <CardTitle className="text-lg font-semibold text-gray-800 flex items-center gap-3">
          ...
          {loading && (
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-500 ml-auto"></div>
          )}
        </CardTitle>
      </CardHeader>
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              activities.map((activity, index) => (
                <div 
                  key={activity.id} 
                  className="flex items-start space-x-3 p-4 rounded-lg bg-white/70 hover:bg-white/90 transition-all duration-200 animate-fade-in border border-gray-100/50"
